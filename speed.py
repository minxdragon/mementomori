def adjust_gcode_speeds(input_gcode, output_gcode, repositioning_speed=5000, plotting_speed=50):
    """
    ...
    with open(input_gcode, 'r') as infile, open(output_gcode, 'w') as outfile:
        for line in infile:
            if line.startswith("G00"):  # Repositioning move
                outfile.write(f"{line.strip()} F{repositioning_speed}\n")
            elif line.startswith("G01"):  # Plotting move
...
# Call the function to adjust the G-code speeds
adjust_gcode_speeds("output.gcode", "modified_output.gcode", 5000, 50)
